...
export class BotController {
  ...
  async changeDaxod(ctx: Context, message: string) {
    try {
      const userId = ctx.from?.id;
      let newDaxod: number = 0;
  ...
  async addRasxod(ctx: Context, message: string) {
    try {
      const userId = ctx.from?.id;
      const user = await User.findOne({ userId });
      ...
      if (ras === 0) {
        ctx.reply(
          `Rasxod qilgan summa kiritilmagan, \nKiritish summa: ${ras} so'm`
        );
      }
