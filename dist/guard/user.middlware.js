...
const phoneKeyboard = new grammy_1.Keyboard()
...
const checkUser = async (ctx, next) => {
    ...
    let user = await db_schema_1.User.findOne({ userId });
    // 1️⃣ Yangi user yaratish
    if (!user) {
        user = await db_schema_1.User.create({
            userId,
    ...
    if (ctx.message?.contact) {
        await db_schema_1.User.findOneAndUpdate({ userId }, { phone: ctx.message.contact.phone_number });
        await ctx.reply("✔️ Telefon raqamingiz qabul qilindi!\n\nEndi oylik oladigan sanani tanlang:", { reply_markup: generateDaysKeyboard() });
        return;
    }
